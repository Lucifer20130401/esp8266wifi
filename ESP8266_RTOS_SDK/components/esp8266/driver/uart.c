...
{
    uart_obj_t *p_uart = (uart_obj_t *)param;//获取传递过来的参数
    uint8_t uart_num = p_uart->uart_num;//获取端口号
    uart_dev_t *uart_reg = UART[uart_num];//指针指向寄存器
    int rx_fifo_len = uart_reg->status.rxfifo_cnt;//获取FIFO字节数
    uint8_t buf_idx = 0;
    uint32_t uart_intr_status = UART[uart_num]->int_st.val;//获取中断状态标志寄存器
    uart_event_t uart_event;
    BaseType_t task_woken = 0;

    while (uart_intr_status != 0x0)//如果有中断？
    {
        uart_select_notif_t notify = UART_SELECT_ERROR_NOTIF;

        buf_idx = 0;
        uart_event.type = UART_EVENT_MAX;//先设置为MAX

        if (uart_intr_status & UART_TXFIFO_EMPTY_INT_ST_M)//如何发送的FIFO是空的
        {
            uart_clear_intr_status(uart_num, UART_TXFIFO_EMPTY_INT_CLR_M);//清中断标志
            uart_disable_intr_mask(uart_num, UART_TXFIFO_EMPTY_INT_ENA_M);//禁止发送结束中断

            // TX semaphore will only be used when tx_buf_size is zero.
            if (p_uart->tx_waiting_fifo == true && p_uart->tx_buf_size == 0)//如果有数据要发送，且缓冲区大小被设置为0,即禁用。
            {
                p_uart->tx_waiting_fifo = false;
                xSemaphoreGiveFromISR(p_uart->tx_fifo_sem, &task_woken);//放出二值信号量。

                if (task_woken == pdTRUE)//这里是什么意思？ 给出信号量后，
                //函数会判断队列里面有没有比进入中断之前更高优先级的任务在排队。如果有，则返回pdTRUE. 用户通过taskYIELD()来为出中断前切换这个高优先级任务。
                {
                    portYIELD_FROM_ISR();//为任务切换做准备
                }
            }
            else
            {
                // We don't use TX ring buffer, because the size is zero.
                if (p_uart->tx_buf_size == 0)//如果设置了缓冲区为0
                {
                    continue;
                }

                int tx_fifo_rem = UART_FIFO_LEN - UART[uart_num]->status.txfifo_cnt;//计算还有多少字节可以放到FIFO？？？？
